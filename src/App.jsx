...
function App() {
  ...
  const [loading, setLoading] = useState(false);        // Sortie loading
  const [logLoading, setLogLoading] = useState(false);  // Log loading
  ...
  return (
    <div className="p-6 font-sans text-center max-w-4xl mx-auto">
      <img className="mb-3" src={mmcd} alt="App banner" />
      <h1 className="text-3xl font-bold mb-4 text-green-700">Sortie Tracker</h1>

      {/* Sortie (weather) button */}
      <button
        onClick={handleFetchWeather}
        className={`${
          loading ? "bg-gray-400 cursor-not-allowed" : "bg-green-700 hover:bg-green-800"
        } text-white px-6 py-2 rounded shadow inline-flex items-center justify-center gap-2`}
      ...
      {/* Log-only button with matching loading behavior */}
      <button
        onClick={handleSortieOnly}
        className={`ml-4 ${
          logLoading ? "bg-gray-400 cursor-not-allowed" : "bg-yellow-600 hover:bg-yellow-700"
        } text-white px-8 py-2 rounded shadow inline-flex items-center justify-center gap-2`}
        disabled={logLoading || loading}
      >
        {logLoading ? (
          <>
            <svg
